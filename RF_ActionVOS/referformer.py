...
class ReferFormer(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, 
        ...
        # add an extra cls head
        self.positive_cls = positive_cls
        if self.positive_cls:
            self.positive_cls_embed = nn.Linear(hidden_dim, 1)
        ...
        if self.positive_cls:
            self.positive_cls_embed.bias.data = torch.ones(1) * bias_value
            self.positive_cls_embed = _get_clones(self.positive_cls_embed, num_pred)
        # Build Text Encoder
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        if self.positive_cls:
            outputs_positives = []
            for lvl in range(hs.shape[0]):
                outputs_positive = rearrange(hs[lvl], '(b t) q c -> b t q c', b=b, t=t)
                outputs_positive = torch.mean(outputs_positive,dim=(1,2))
                outputs_positive = self.positive_cls_embed[lvl](outputs_positive)
                outputs_positives.append(outputs_positive)
            outputs_positives = torch.stack(outputs_positives) #l b 1
            #print(outputs_positives.shape)
            out['pred_positives'] = outputs_positives[-1]
        ...
        if self.aux_loss:
            if self.positive_cls:
                out['aux_outputs'] = self._set_aux_loss_pos(outputs_class, outputs_coord, outputs_seg_masks, outputs_positives)
            else:    
    ...
    @torch.jit.unused
    def _set_aux_loss_pos(self, outputs_class, outputs_coord, outputs_seg_masks, outputs_positives):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [{"pred_logits": a, "pred_boxes": b, "pred_masks": c, "pred_positives": d} 
                for a, b, c, d in zip(outputs_class[:-1], outputs_coord[:-1], outputs_seg_masks[:-1], outputs_positives[:-1])]
...
def build(args):
    ...
    model = ReferFormer(
        ...
        positive_cls=args.use_positive_cls
    )
    ...
    if args.use_positive_cls: # an extra cls head for positive classification
        weight_dict['loss_positive_labels'] = args.cls_loss_coef
    # TODO this is a hack
    if args.aux_loss:
        aux_weight_dict = {}
    ...
    losses = ['labels', 'boxes']
    if args.use_positive_cls:
        losses += ['positive_labels']
    if args.masks:
        if args.use_weights:
            losses += ['weighted_masks']
        else:
            losses += ['masks']
