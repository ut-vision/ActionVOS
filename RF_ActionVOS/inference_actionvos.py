...
def main(args):
    ...
    img_folder = os.path.join(root, "JPEGImages_Sparse", split)
    meta_file = os.path.join(root, "ImageSets", args.expression_file)
...
def sub_processor(lock, pid, args, data, save_path_prefix, save_visualize_path_prefix, img_folder, video_list):
    ...
    # get palette
    palette_img = '../annotations/00000.png'
    ...
    for video in video_list:
        ...
        # 2. For each expression, add by oyly, join results
        anno_logits = []  
        anno_masks = []
        for i in range(num_expressions):
            ...
            # following inference_davis.py, we set max length to 36
            # for each clip
            all_pred_logits = []
            all_pred_masks = []
            for clip_id in range(0, video_len, 36):
                ...
                max_scores, max_ind = max_scores.max(-1)     # [1,]
                ...
                pred_logits = pred_logits[range(clip_len), max_inds]

                # use positive label classification to filter negative objects
                if args.use_positive_cls:
                    if outputs['pred_positives'][0].sigmoid() < args.pos_cls_thres:
                        pred_masks.zero_()
        ...
        # save results
        anno_save_path = os.path.join(save_path_prefix, video)
