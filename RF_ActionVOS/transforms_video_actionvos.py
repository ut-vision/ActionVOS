...
def crop(clip, target, region):
    ...
    if "weights" in target:
        # FIXME should we update the area here if there are no boxes?
        target['weights'] = target['weights'][:, i:i + h, j:j + w]
...
def hflip(clip, target):
    ...
    if "weights" in target:
        target['weights'] = target['weights'].flip(-1)
...
def resize(clip, target, size, max_size=None):
    ...
    if "weights" in target:
        if target['weights'].shape[0]>0:
            target['weights'] = interpolate(
                target['weights'][:, None].float(), size, mode="nearest")[:, 0]
        else:
            target['weights'] = torch.ones((target['weights'].shape[0],h,w))
...
def pad(clip, target, padding):
    ...
    if "weights" in target:
        target['weights'] = torch.nn.functional.pad(target['weights'], (0, padding[0], 0, padding[1]), value=1)
